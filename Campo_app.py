...
# Lista de vendedores desde la imagen
VENDEDORES_LISTA = [
    "ESTHEFANO PRETELL BAZAN",
    "JULIO IVAN REBAZA URTECHO",
    "MARIA NOEMI BLAS VILLARRUEL",
    "YESSICA MAGALY SOTO ZAVALETA",
    "CARLOS MANUEL QUIROZ DIAZ",
    "IVAN GERONIMO CRUZ",
    "HEISER LOPEZ CRUZADO",
    "MILAGROS SOLEDAD ESQUERRE GARCIA",
    "MAYCOLL SOLORZANO VILLACORTA",
    "MARIA CRISTHINA PAREDES NEYRA",
    "JESUS SMITH ARGOMEDO ZAVALETA",
    "DICK RONNI MENDOCILLA DOMINGUEZ",
    "MAYDA CAROLINA PEREZ MENDOZA",
    "CARLOS JONATAN VALDERRAMA CRUZADO",
    "NUNEZ REYES KARLA PATRICIA",
    "JORGE RUBEN TABOADA PRINCIPE",
    "ANGEL SERAFIN ROMERO ROJAS",
    "DAYANA YAJARUMI CASTILLO AMASIFUEN",
    "LUCIA ELIZABETH VILLANUEVA DOMINGUEZ",
    "JHORDAN MURGA ORTIZ",
    "ELISA FLOR CHIGUALA DE LA CRUZ",
    "RODMAN JERSON DE LA CRUZ RODRIGUEZ",
    "DOUGLAS ALFONSO FLORES VARGAS",
    "JEAN POOL RAMIREZ RODRIGUEZ",
    "BERTIN JUNIOR BACA BARRIGA",
    "FRANKLIN ROYMAR VILLOSLADA DIAZ",
    "ALBERTO ESTEBAN CARLOS PONCE",
    "MARIA ESTEFANY RODRIGUEZ MENDOZA",
    "CAROLINA MIRANDA CHICO",
    "ROSANGELA GRACIELA GUANILO RAMOS",
    "JHON DARWIN CONTRERAS DIAZ",
    "GIAMPIERO VLADIMIR VILLAFANA BECERRA",
    "FRANKLIN JERSON VELASQUEZ CUSQUISIBAN",
]
...
if st.session_state.pagina == "formulario":
    ...
    if st.session_state.gps_lat and st.session_state.gps_lon:
        ...
        # ── Ajuste manual directo (sin buscador) ─────────────────────────────
                    ...
                    with st.spinner("Obteniendo nombre del lugar..."):
                        try:
                            ...
                            r = _req.get(
                                ...
                                params={"lat": lat_nuevo, "lon": lon_nuevo, "format": "json",
                                        "accept-language": "es", "zoom": 18},
                                headers={"User-Agent": "MDZ-SupervisionApp/1.0"}, timeout=6,
                            )
                            if r.status_code == 200:
                                addr = r.json().get("address", {})
                                nombre = (addr.get("road") or addr.get("pedestrian") or
                                          addr.get("neighbourhood") or addr.get("suburb") or
                                          addr.get("town") or addr.get("city") or "Ubicación seleccionada")
                    ...
                    if not lugar_final:
                        with st.spinner("Obteniendo nombre..."):
                            try:
                                ...
                                r = _req.get(
                                    "https://nominatim.openstreetmap.org/reverse",
                                    params={"lat": lat_click, "lon": lon_click, "format": "json",
                                            "accept-language": "es", "zoom": 18},
                                    headers={"User-Agent": "MDZ-SupervisionApp/1.0"}, timeout=6,
                                )
                                if r.status_code == 200:
    ...
    with st.form("form_visita", clear_on_submit=False):
        ...
        with col_v1:
            # Selectbox con lista de vendedores
        ...
        # Eliminado OREO_CLASICA_ROLLO según solicitud
        productos_foco = {
        ...
        if submitted:
            if not codigo_pdc or not nombre_cliente or giro_negocio == "Selecciona...":
                ...
                # Guardar múltiples fotos con nombre del cliente
                img_paths = []
                ...
                registro = {
                    ...
                    **{k: int(v) for k, v in pf_vals.items()},
                    # Asegurar que OREO_CLASICA_ROLLO no se guarde
...
# ═══════════════════════════════════════════
    ...
    # ── KPIs principales (Efectividad primero) ─────────────────────────────
    ...
    # ── FILA 1: Exhibidores + Giro negocio + Terceros ─────────────────────
    ...
    with col_g2:
        ...
        if "Giro_Negocio" in df_f.columns:
            ...
            fig_giro = go.Figure(go.Pie(
                ...
                hole=0,
                marker=dict(colors=PALETTE_GIRO[:len(df_giro)], line=dict(color="white", width=2)),
    ...
    # ── FILA 2: Contaminación + Tipo de negocio por exhibidor ─────────────
    col_cont1, col_cont2 = st.columns(2)

    with col_cont1:
        st.markdown("#### ⚠️ Contaminación de Exhibidores")
        cont_data = []
        for col_c, label_c in [
        ...
        if cont_data:
            ...
            fig_cont = px.bar(df_cont, x="Exhibidor", y="Cantidad", color="Estado", text="Etiqueta",
                barmode="group",
                color_discrete_map=PALETTE_CONT,
                category_orders={"Estado": ["Contaminado", "Limpio"]})
    ...
    with col_cont2:
        ...
        exhib_giro_data = []
        for col_ex, label_ex in exhib_cols.items():
    ...
    if vis_data:
        ...
        fig_vis = px.bar(df_vis, x="Exhibidor", y="Cantidad", color="Nivel", text="Etiqueta",
            barmode="group",
            color_discrete_map={"Alta": "#4CAF50", "Media": "#FFC107", "Baja": "#e05252"},
    ...
    with col_g4:
        ...
        fig_efec.update_layout(**base_layout("EFECTIVIDAD DE VISITAS"), showlegend=False,
            xaxis=dict(tickfont=dict(size=FONT_SIZE_AXIS)),
            yaxis=dict(tickfont=dict(size=FONT_SIZE_AXIS)))
    ...
    def render_presencia_bar(productos_dict, df_source, total_vis, palette):
        ...
        # Asignar color único a cada producto
    ...
    with tab_biscuits:
        render_presencia_bar({
            ...
            "FIELD_CC":"FIELD CC","FIELD_DP":"FIELD DP","FIELD_VAIN":"FIELD VAIN",
            "CLUB_SOCIAL_TRA":"CLUB SOCIAL TRA",
        }, df_f, total_visitas, PALETTE_PRODUCTOS)
    with tab_foco:
        render_presencia_bar({
            "OREO_FRESA_PACK":        "OREO FRESA (Pack)",
            "OREO_FRESA_ROLLO":       "OREO FRESA (Rollo)",
    ...
    with dcol2:
        # Dashboard operativo en Excel con KPIs y tablas resumen
        try:
            import openpyxl
            ...
            # Estilos
            def cell_style(ws, row, col, value, bold=False, bg=None, font_size=11, align="center", wrap=False, font_color="000000"):
            ...
            c2.font = Font(size=12, color="555555")
            ...
            row_after_tk = row_tk + 2 + len(ticket_calc) + 1

            # ── RESUMEN EXHIBIDORES ───────────────────────────────────────
            ws.merge_cells(f"A{row_after_tk}:H{row_after_tk}")
            c_ex = ws.cell(row=row_after_tk, column=1, value="COLOCACIÓN DE EXHIBIDORES")
            c_ex.font = Font(bold=True, size=13, color="FFFFFF")
            c_ex.fill = PatternFill("solid", fgColor="FF7F0E")
            c_ex.alignment = Alignment(horizontal="center", vertical="center")
            ws.row_dimensions[row_after_tk].height = 22

            ex_headers = ["Exhibidor", "Cantidad", "% del Total"]
            for ci, h in enumerate(ex_headers, 1):
                c = ws.cell(row=row_after_tk+1, column=ci, value=h)
                c.font = Font(bold=True, size=11, color="FFFFFF")
                c.fill = PatternFill("solid", fgColor="7b5ea7")
                c.alignment = Alignment(horizontal="center", vertical="center")
                c.border = border

            for ri, row_e in enumerate(data_exhib):
                r = row_after_tk + 2 + ri
                ws.cell(row=r, column=1, value=row_e["Exhibidor"]).border = border
                ws.cell(row=r, column=2, value=row_e["Cantidad"]).border = border
                ws.cell(row=r, column=3, value=f"{row_e['Pct']}%").border = border
                ws.row_dimensions[r].height = 18

            row_after_ex = row_after_tk + 2 + len(data_exhib) + 1

            # ── CONTAMINACIÓN ─────────────────────────────────────────────
            ws.merge_cells(f"A{row_after_ex}:H{row_after_ex}")
            c_ct = ws.cell(row=row_after_ex, column=1, value="CONTAMINACIÓN DE EXHIBIDORES")
            c_ct.font = Font(bold=True, size=13, color="FFFFFF")
            c_ct.fill = PatternFill("solid", fgColor="e05252")
            c_ct.alignment = Alignment(horizontal="center", vertical="center")
            ws.row_dimensions[row_after_ex].height = 22

            ct_headers = ["Exhibidor", "Contaminados", "% Contaminados", "Limpios", "% Limpios"]
            for ci, h in enumerate(ct_headers, 1):
                c = ws.cell(row=row_after_ex+1, column=ci, value=h)
                c.font = Font(bold=True, size=11, color="FFFFFF")
                c.fill = PatternFill("solid", fgColor="e05252")
                c.alignment = Alignment(horizontal="center", vertical="center")
                c.border = border

            ri_cont = 0
            for col_c2, label_c2 in [
                ("CONT_LEGOS_GC", "LEGOS G&C"),
                ("CONT_TOBOGAN_RITZ_OREO", "TOBOGÁN Ritz/Oreo"),
                ("CONT_EXHIB_KIWI", "EXHIB KIWI"),
            ]:
                if col_c2 in df_f.columns:
                    serie_c2 = pd.to_numeric(df_f[col_c2], errors="coerce").fillna(0)
                    cont_si2  = int(serie_c2.sum())
                    cont_no2  = len(serie_c2) - cont_si2
                    pct_si2   = round(cont_si2 / len(serie_c2) * 100, 1) if len(serie_c2) > 0 else 0
                    pct_no2   = round(cont_no2 / len(serie_c2) * 100, 1) if len(serie_c2) > 0 else 0
                    r2 = row_after_ex + 2 + ri_cont
                    ws.cell(row=r2, column=1, value=label_c2).border = border
                    ws.cell(row=r2, column=2, value=cont_si2).border = border
                    ws.cell(row=r2, column=3, value=f"{pct_si2}%").border = border
                    ws.cell(row=r2, column=4, value=cont_no2).border = border
                    ws.cell(row=r2, column=5, value=f"{pct_no2}%").border = border
                    ws.row_dimensions[r2].height = 18
                    ri_cont += 1

            row_after_ct = row_after_ex + 2 + ri_cont + 1

            # ── PRESENCIA BISCUITS ────────────────────────────────────────
            ws.merge_cells(f"A{row_after_ct}:H{row_after_ct}")
            c_pr = ws.cell(row=row_after_ct, column=1, value="PRESENCIA DE PRODUCTOS")
            c_pr.font = Font(bold=True, size=13, color="FFFFFF")
            c_pr.fill = PatternFill("solid", fgColor="1a1a1a")
            c_pr.alignment = Alignment(horizontal="center", vertical="center")
            ws.row_dimensions[row_after_ct].height = 22
            ...
            pr_headers = ["Producto", "Visitas con Presencia", "Total Visitas", "Presencia %"]
            for ci, h in enumerate(pr_headers, 1):
                c = ws.cell(row=row_after_ct+1, column=ci, value=h)
                c.font = Font(bold=True, size=11, color="FFFFFF")
            ...
            ri_pr = 0
            for key_p, label_p in todos_productos.items():
                if key_p in df_f.columns:
                    ...
                    r_p = row_after_ct + 2 + ri_pr
                    ws.cell(row=r_p, column=1, value=label_p).border = border
                    ws.cell(row=r_p, column=2, value=cnt_p).border = border
                    ws.cell(row=r_p, column=3, value=total_visitas).border = border
                    ws.cell(row=r_p, column=4, value=f"{pct_p}%").border = border
                    ws.row_dimensions[r_p].height = 18
                    ri_pr += 1

            # Ajustar anchos
            col_widths = [30, 18, 18, 18, 18, 18, 18, 18]
