...
if st.session_state.pagina == "formulario":
    ...
    with st.form("form_visita", clear_on_submit=False):
        ...
        with v1:
            vis_legos = st.radio("LEGOS G&C", options=[1, 2, 3],
                format_func=lambda x: {1: "1 - Alta", 2: "2 - Media", 3: "3 - Baja"}[x], horizontal=True, key="vl")
        with v2:
            vis_tobogan = st.radio("TOBOGÁN (Ritz/Oreo)", options=[1, 2, 3],
                format_func=lambda x: {1: "1 - Alta", 2: "2 - Media", 3: "3 - Baja"}[x], horizontal=True, key="vt")
        with v3:
            vis_kiwi = st.radio("EXHIB KIWI", options=[1, 2, 3],
                format_func=lambda x: {1: "1 - Alta", 2: "2 - Media", 3: "3 - Baja"}[x], horizontal=True, key="vk")
...
# ═══════════════════════════════════════════
    ...
    # FILA 2: Efectividad (Concreto vs No) + Mapa
    ...
    cols_tabla = ["Fecha", "Codigo_PDC", "Nombre_Cliente", "Giro_Negocio", "Vendedor", "Zona",
                  "Efectividad_Soles", "Ticket_Promedio", "Tiempo_PDC", "Visibilidad_Legos",
                  "Colocacion_Terceros", "Marca_Tercero"]
